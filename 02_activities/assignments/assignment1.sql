...
SUM(cp.quantity*cp.cost_to_customer_per_qty) as  customer_spend
FROM customer as c
INNER JOIN customer_purchases as cp
	ON c.customer_id = cp.customer_id
GROUP BY c.customer_id
HAVING customer_spend > 2000
...
SELECT customer_id, 
  STRFTIME('%m', purchase_date) AS month,
  STRFTIME('%Y', purchase_date) AS year,
  SUM(quantity * cost_to_customer_per_qty) as customer_spend
FROM customer_purchases
